...
const cmd = {
  ...
  builder: function (yargs) {
    return yargs
      ...
      .example(chalk.yellow('leetcode virtual weekly-contest-175 --view'), 'Get Contest Detail')
...
// @TODO Prettify Printing
function printQuestions(data) {
  for (let i = 0; i < data.length; i++)
    console.log(data[i]['problem']['fid'] + ' ' + data[i]['problem']['state'] + ' ' + data[i]['credit'] + ' ' + data[i]['problem']['slug']);
}
...
function getProblemList(questions) {
  ...
  for (let i = 0; i < questions.length; i++) {
    ...
    const title_slug = questions[i]['title_slug'];
    problems.push({ "qid": qid, "credit": credit, "title_slug": title_slug })
    // problems[title_slug] = {"qid":qid, "credit":credit}
  }
  return problems;
}

function handleArguments(err, problems) {
  const argv = session.argv

  // debug(problems)

  if (argv.view)
    printQuestions(problems)
...
function contestApiInfo(argv, callback) {
  // @TODO Check json object/response from cache
  const opts = core.makeOpts('GET', config.sys.urls.vcontestapi.replace('$contest', argv.contestname), config.sys.urls.vcontestpage.replace('$contest', session.argv.contestname));

  request(opts, function (error, response) {
    if (error) throw new Error(error);
    // @TODO Update Cache by response.body
...
cmd.handler = function (argv) {
  ...
  contestApiInfo(argv, runVirtual);
};
