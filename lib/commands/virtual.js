...
function handleArguments(e, problems) {
  ...
  if (argv.view || (!argv.start && !argv.end && argv.question == -1 && argv.filename == '' && !argv.submit))
...
function printRank(body){
  var output = {rank : body.my_rank.rank, score : body.my_rank.score, solved: []};
  for(let id of body.my_solved)
    output.solved.push({id: id, fail_count: body.my_submission[id].fail_count, solved: h.statusToName(body.my_submission[id].status)});
...
function showMyRank() {
  ...
  request(opts, function (error, response) {
    ...
    // @TODO Only if contest is running
    return printRank(body);
  });
}
