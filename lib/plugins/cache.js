...
plugin.getContest = function(contestSlug, cb) {
  const k = h.KEYS.contest(contestSlug);
  const _contest = cache.get(k);
  if(_contest){
    log.debug('cache hit: ' + k + '.json');
    return cb(null, _contest);
  }

  plugin.next.getContest(contestSlug, function(e, _contest) {
    if (e) return cb(e);

    plugin.saveContest(_contest);
    return cb(null, _contest);
  });
}

plugin.saveContest = function(contest) {
  return cache.set(h.KEYS.contest(contest), contest);
}
