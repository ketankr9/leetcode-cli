...
plugin.getContest = function(contestSlug, cb) {
  log.debug('running leetcode.getContest');
  const user = session.getUser();
  const opts = plugin.makeOpts(config.sys.urls.vcontestapi.replace('$contest', contestSlug));

  const spin = h.spin('Downloading ' + contestSlug);
  request.get(opts, function (e, resp) {
    spin.stop();
    e = plugin.checkError(e, resp, 200);
    if (e) return cb(e);
    const body = JSON.parse(resp.body);
    
    if(body.error && body.error == 'Contest does not exist.')
      return cb(body.error);
    if(!body.questions || body.questions.length == 0)
      return cb('Contest hasn\'t started yet.');
...
plugin.testVirtualProblem = function(problem, cb) {
  log.debug('running leetcode.testVirtualProblem');
  const opts = plugin.makeOpts(config.sys.urls.vtest.replace('$slug', problem.slug).replace('$contest', problem.contest));
  opts.body = {data_input: problem.testcase};
...
plugin.submitVirtualProblem = function(problem, cb) {
  log.debug('running leetcode.submitVirtualProblem');
  const opts = plugin.makeOpts(config.sys.urls.vsubmit.replace('$slug', problem.slug).replace('$contest', problem.contest));
  opts.body = {judge_type: 'large'};
